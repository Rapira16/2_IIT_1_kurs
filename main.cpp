...
std::vector<Individual> createNewPopulation(const std::vector<Individual>& population, int populationSize) {
    ...
    for (int i = 0; i < offspringCount; ++i) {
        ...
        if (ratio < 0.1) {  // Вероятность мутации - 10%
...
int main() {
    ...
    int populationSize = checkInput();  // Размер начальной популяции
    std::cout << "Enter amount of iterations (ends with 00)" << std::endl;
    int numIterations = checkInput();  // Количество итераций генетического алгоритма
    std::string extreme;
    while (true) {
